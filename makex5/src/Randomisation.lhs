...
\begin{code}
doRandomR :: (Monad m, Random a, RandomGen g) 
          => (a, a) -> g 
          -> m (a, g)
...
\subsubsection{Randomise cycle of fixed set of numbers}


We sometime want to repeatedly choose 
from a small repertoire of numbers (e.g., $0,\dots,N$),
but we don't want these to cycle as presented.
We'd prefer that each cycle is randomised:

\begin{code}
randomCycle :: (Eq a, Random a) 
            => Int -> Int -> (a,a) 
            -> StdGen -> ([a],StdGen)
randomCycle 0 _ _ g  =  ([],g)
randomCycle c n (lo,hi) g
  = let
      (ns',g') = randomRUn n (lo,hi) g
      (ns'',g'')  = randomCycle (c-1) n (lo,hi) g'
    in (ns'++ns'',g'')

doRandomCycle :: (Monad m, Eq a, Random a) 
              => Int -> Int -> (a,a) 
              -> StdGen -> m ([a],StdGen)
doRandomCycle c n (lo,hi) g = return $ randomCycle c n (lo,hi) g
\end{code}
